...
async def async_main():
    ...
    with ClusterClient(
        ...
        await pool_handle.wait_for_ready(expected=len(pool_ids))
        ...
        for idx, pool_id in enumerate(pool_ids):
            await pool_handle.invoke_all(
                ("generate", ("hello",), None), actor_id=pool_id
            )
            logger.info(f"Pool {idx + 1}/{len(pool_ids)} warmed up")
        ...
        tasks = [asyncio.create_task(_run_pool(pid, inference_msg)) for pid in pool_ids]
        try:
