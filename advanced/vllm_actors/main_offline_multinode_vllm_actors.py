...
from ensemble_launcher.inference import MultiNodeVLLMInference, copy_model
...
async def async_main():
    ...
    for i in range(len(nodes)):
        ...
        actor = MultiNodeVLLMInference(
            ...
            use_cached_modelinfo=True,
            model_info_cache=f"{os.getcwd()}/vllm_cache",
        )
        actors.append(actor)
        actor_tasks.append(
            actor.create_task(
    ...
    with ClusterClient(checkpoint_dir=ckpt_dir, checkpoint_timeout=300) as client:
        ...
        for i, actor in enumerate(actors):
            ...
            if handle is not None:
                await handle.open()
                await handle.send(("generate", (chunks[i],), None))
        ...
        while len(all_results) < len(prompts) and len(done) < len(actor_futures):
            for i, handle in enumerate(handles):
                ...
                if actor_futures[i].done() and actor_futures[i].exception() is not None:
                    logger.info(f"actor {i} died with: {actor_futures[i].exception()}")
                    done.add(i)
