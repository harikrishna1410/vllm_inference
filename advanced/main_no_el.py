...
def main():
    ...
    vllm_proc = subprocess.Popen(
        [
            ...
            "1",
            "--cpu-bind",
            "list:" + ":".join(map(str, range(1, args_dict["ngpus_per_model"] + 1))),
            "bash",
            "-c",
            f"{os.getcwd()}/start_vllm_server.sh 0 {args_dict['port']} {args_dict['ngpus_per_model']} {args_dict['model']} {cache_dir} {args_dict['tmp_dir']}",
        ],
    )
    logger.info("vllm server processes launched (pid=%d)", vllm_proc.pid)
